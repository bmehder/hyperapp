...
const Data = {
	formGroupsData: state => [
		{
			label: 'Enter the average amount of a sale for your business:',
			value: state.avgSale,
		...
		{
			label: 'Enter the percentage of repeat business:',
		...
		{
			type: 'text',
			label: 'Cool, each new client you get is worth this amount:',
		...
		{
			label: 'Enter percentage of leads you convert to a paying client:',
		...
		{
			type: 'text',
			label: 'So then, each new lead is worth this to your business:',
			value: Helpers.toUSD(
		...
		{
			type: 'text',
			label: 'So, this is how much more you could earn per month:',
			value: Helpers.toUSD(
...
const Views = {
	...
	default: state =>
		html.div({ class: 'flow' }, [
			...Data.formGroupsData(state).map(Views.formGroup),
